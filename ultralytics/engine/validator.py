    ...
    def __call__(self, trainer=None, model=None, **kwargs):
        ...
        if self.training:
            ...
            if trainer.world_size > 1:
                # torch < 1.11 has no ReduceOp.AVG: fall back to SUM + manual divide
                avg_op = getattr(dist.ReduceOp, "AVG", None)
                for v in loss.values():
                    dist.reduce(v, dst=0, op=avg_op or dist.ReduceOp.SUM)
                    if avg_op is None and RANK == 0:
