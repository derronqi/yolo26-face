...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, ltrb_norm: str = "imgsz"):
        """Initialize the BboxLoss module with regularization maximum and DFL settings.

        Args:
            reg_max (int): DFL bins; reg_max <= 1 disables DFL and uses direct regression.
            ltrb_norm (str): Normalization for the direct-regression L1 loss, "imgsz" (default) or
                "gtwh" (per-GT-box width/height, scale-invariant across face sizes).
        """
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.ltrb_norm = ltrb_norm
    ...
    def forward(
        ...
        if self.dfl_loss:
            ...
            if self.ltrb_norm == "gtwh":
                # scale-invariant: normalize ltrb (pixels) by the GT box's own width/height
                gt_wh = (target_bboxes[..., 2:4] - target_bboxes[..., 0:2]).clamp_min(1e-3) * stride  # (b, N, 2)
                target_ltrb = target_ltrb * stride
                target_ltrb[..., 0::2] /= gt_wh[..., 0:1]
                target_ltrb[..., 1::2] /= gt_wh[..., 1:2]
                pred_dist[..., 0::2] /= gt_wh[..., 0:1]
                pred_dist[..., 1::2] /= gt_wh[..., 1:2]
            else:
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.assigner = TaskAlignedAssigner(
            ...
            topk2=tal_topk2,
            stal_mult=getattr(h, "stal_mult", 1.0),
        )
        self.bbox_loss = BboxLoss(m.reg_max, getattr(h, "ltrb_norm", "imgsz")).to(device)
